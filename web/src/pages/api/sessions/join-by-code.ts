...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const { code, name, bot, password } = body;
    if (!code || !name || !bot) {
      return new Response(JSON.stringify({ error: 'code, name, and bot required' }), { status: 400 });
    }

    const redis = getRedis();
    const sessionId = await redis.get(`session-code:${code.toUpperCase()}`);
    if (!sessionId) {
      return new Response(JSON.stringify({ error: 'Invalid join code' }), { status: 404 });
    }

    const raw = await redis.get(`sessions:${sessionId}`);
    if (!raw) {
      return new Response(JSON.stringify({ error: 'Session expired' }), { status: 404 });
    }
