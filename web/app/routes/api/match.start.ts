...
export const Route = createFileRoute('/api/match/start')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        let body: any
        ...
        if (!botA || !botB) {
          return new Response(
            JSON.stringify({ error: 'botA and botB are required' }),
        ...
        try {
          ...
          if (!doRes.ok) {
            ...
            console.error(
              `[match/start] DO start failed (${doRes.status}): ${detail}`,
            )
            return new Response(
              JSON.stringify({ error: 'Engine start failed', detail }),
