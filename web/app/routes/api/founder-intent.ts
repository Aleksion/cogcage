...
export const Route = createFileRoute('/api/founder-intent')({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          ...
          appendOpsLog({ route: '/api/founder-intent', level: 'info', event: 'founder_intent_health_check', requestId, status: 'ok', storage: 'redis', durationMs: Date.now() - startedAt });
          try {
