...
export const Route = createFileRoute('/shed')({
  head: () => ({
    meta: [
      { title: 'The Molt Pit — The Shed' },
      { name: 'description', content: 'Your command deck. Review your Crustie and enter the next molt.' },
    ],
  }),
  component: ForgePage,
})
...
function ForgeContent() {
  ...
  useEffect(() => {
    if (!isLoading && !isAuthenticated) {
      navigate({ to: '/sign-in', search: { returnTo: '/shed' } })
    }
  ...
  const username = player?.username || 'Crustie'
        ...
        <div className="forge-grid">
          ...
          {shells && shells.length > 0 ? (() => {
            ...
            return (
              <div className="forge-panel forge-crawler-preview">
                <div className="forge-bot-wrap">
                  ...
                  <div className="forge-bot-art" style={{
                    fontSize: '1.65rem',
                    gap: '0.35rem',
                    display: 'flex',
                    flexWrap: 'wrap',
                    ...
                    {shell.cards.slice(0, 8).map((cardId, i) => (
                      <span key={i}>{getCard(cardId)?.icon ?? '•'}</span>
                    ))}
                  </div>
                </div>
                <h2 className="forge-crawler-name">{shell.name}</h2>
                <div className="forge-mold-label">
                  {shell.cards.length} claws loaded&ensp;
                ...
                <div className="forge-ctas">
                  <Link to="/pit" className="forge-cta forge-cta-primary">
                    &#9889; Find a Molt
                  </Link>
                  <Link to="/mise" className="forge-cta forge-cta-secondary">
                    &#9998; Tune Molt
                  </Link>
                </div>
              </div>
            ...
            <div className="forge-panel forge-crawler-preview">
              <div className="forge-bot-wrap">
                <div className="forge-bot-glow" />
                <div className="forge-bot-art" style={{ fontSize: '0.95rem', background: 'rgba(0,229,255,0.08)', color: '#00E5FF', letterSpacing: '1px', textTransform: 'uppercase', fontFamily: "'IBM Plex Mono', monospace" }}>No Active Molt</div>
              </div>
              <h2 className="forge-crawler-name">PREP YOUR CRUSTIE</h2>
              <div className="forge-mold-label" style={{ maxWidth: 240 }}>
                No active molt yet. Build your first molt in The Mise.
              </div>
              <div className="forge-ctas">
                <Link to="/mise" className="forge-cta" style={{
                  background: '#FFD600',
                  color: '#000',
          ...
          <div>
            ...
            <div className="forge-panel">
              <div className="forge-section-title">
                Active Scuttles{openTanks && openTanks.length > 0 ? ` (${openTanks.length} open)` : ''}
              </div>
              <div className="forge-tanks-strip">
                {(!openTanks || openTanks.length === 0) ? (
                  <div className="forge-empty">No open scuttles right now. Start one in The Pit.</div>
                ) : (
                  openTanks.map((tank) => (
                    <div key={tank._id} className="forge-tank-item">
                      <span className="forge-tank-host">&#9654; Scuttle</span>
...
function RecentMolts() {
  ...
  return (
    <div className="forge-empty">No molts yet. Jump into The Pit to start fighting!</div>
  )
}
