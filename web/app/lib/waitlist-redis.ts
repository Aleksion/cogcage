...
export async function redisWriteApiRequestReceipt(receipt: {
  ...
  await r.set(key, value, { ex: IDEMPOTENCY_TTL_SECONDS });
}
...
export async function redisConsumeRateLimit(
  ...
  const safeScope = scope.replace(/[^a-z0-9_-]/gi, '_').slice(0, 40) || 'global';
  const windowKey = `${RATE_LIMIT_PREFIX}${safeScope}:${key}:${windowId}`;
