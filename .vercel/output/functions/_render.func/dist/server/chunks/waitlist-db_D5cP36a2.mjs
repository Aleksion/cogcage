...
function runWithBusyRetry(label, fn) {
  const maxAttempts = 4;
  let lastError;
...
function insertWaitlistLead(lead) {
  const conn = getDb();
...
function insertFounderIntent(intent) {
  const conn = getDb();
  const insert = conn.prepare(`
...
function insertConversionEvent(event) {
  const conn = getDb();
  const insert = conn.prepare(`
...
function consumeRateLimit(ipAddress, route, limit, windowMs) {
  const conn = getDb();
...
function getFunnelCounts() {
  const conn = getDb();
  const waitlist = conn.prepare("SELECT COUNT(*) AS count FROM waitlist_leads").get();
  const founders = conn.prepare("SELECT COUNT(*) AS count FROM founder_intents").get();
  const events = conn.prepare("SELECT COUNT(*) AS count FROM conversion_events").get();
  return {
...
function getReliabilitySnapshot(windowHours = 24) {
  const conn = getDb();
  ...
  const query = conn.prepare(`
    ...
    GROUP BY event_name
  `);
  const rows = runWithBusyRetry("reliability_snapshot", () => query.all(`-${hours} hours`));
...
export { insertFounderIntent as a, getFunnelCounts as b, consumeRateLimit as c, getReliabilitySnapshot as d, insertWaitlistLead as e, ensureRuntimeDir as f, getRuntimeDir as g, insertConversionEvent as i };
