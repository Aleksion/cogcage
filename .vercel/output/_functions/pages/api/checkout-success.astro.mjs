...
function deriveFallbackEventId({ source, email, href, page, tier }) {
...
function safeMetaJson(meta) {
  if (!meta) return void 0;
  try {
    const serialized = JSON.stringify(meta);
    return serialized.length > 4e3 ? JSON.stringify({ truncated: true, preview: serialized.slice(0, 3800) }) : serialized;
  } catch {
...
const POST = async ({ request }) => {
  ...
  const eventId = optionalString(payload.eventId, 180) ?? optionalString(payload.session_id, 180) ?? optionalString(payload.checkout_session_id, 180) ?? deriveFallbackEventId({ source, email, href, page, tier });
  const meta = payload.meta && typeof payload.meta === "object" ? payload.meta : void 0;
  try {
    ...
    return new Response(JSON.stringify({ ok: true, requestId: result.requestId, queued: result.queued || void 0 }), {
...
const GET = async ({ request }) => {
  ...
  const eventId = optionalString(url.searchParams.get("event_id"), 180) ?? optionalString(url.searchParams.get("session_id"), 180) ?? optionalString(url.searchParams.get("checkout_session_id"), 180);
